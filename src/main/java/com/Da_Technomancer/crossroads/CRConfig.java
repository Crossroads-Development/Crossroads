...
//		beamDamageAbsolute = serverBuilder.comment("Whether void and void-potential (death) beams do absolute damage", "Absolute damage ignores potion effects and enchantments").define("beam_damage_absolute", false);
		...
		voltusValue = serverBuilder.comment("FE per tick produced by the Voltus Generator").defineInRange("voltus_power", 50_000, 0, 1_000_000);
...
//		serverSpec.setConfig(serverConfig);
	...
	public static String formatVal(double d, @Nullable Player player){
		return ConfigUtil.formatDouble(d, getNumberFormat(player));
	}
	...
	public static String formatVal(int i, @Nullable Player player){
		return ConfigUtil.formatInteger(i, getNumberFormat(player));
	}
}
