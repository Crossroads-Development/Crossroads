...
//	}
	...
	public static void addLinkInfo(List<Component> tooltip, ILinkTE te){
		...
		if(links.size() == 0){
			tooltip.add(Component.translatable("tt.crossroads.boilerplate.link.none"));
		}else{
			...
			if(totalLinked <= 4){
				for(BlockPos relPos : links){
					...
					tooltip.add(Component.translatable("tt.crossroads.boilerplate.link", linkPos.getX(), linkPos.getY(), linkPos.getZ()));
				}
			}else{
				int printed = 0;
				for(BlockPos relPos : links){
				...
				tooltip.add(Component.translatable("tt.crossroads.boilerplate.link.excess", totalLinked + 1 - printed));
			}
		}
	}
