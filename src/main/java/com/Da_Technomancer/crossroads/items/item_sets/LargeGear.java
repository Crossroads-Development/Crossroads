...
//		}
		...
		//Place the gear
		world.setBlock(pos, CRBlocks.largeGearMaster.defaultBlockState().setValue(CRProperties.FACING, side.getOpposite()), 3);
		...
		for(BlockPos cPos : relSlavePos[side.getAxis().ordinal()]){
			world.setBlock(pos.offset(cPos), CRBlocks.largeGearSlave.defaultBlockState().setValue(CRProperties.FACING, side.getOpposite()), 3);
