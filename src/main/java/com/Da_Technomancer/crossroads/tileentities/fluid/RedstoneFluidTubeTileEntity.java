...
public class RedstoneFluidTubeTileEntity extends TileEntity implements ITickable, IIntReceiver{
		...
		if(world.getBlockState(pos).getValue(EssentialsProperties.REDSTONE_BOOL)){
			for(EnumFacing dir : EnumFacing.values()){
				...
				if(offsetTE != null && offsetTE.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, dir.getOpposite())){
					transfer(offsetTE.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, dir.getOpposite()), ind);
				}
			}
		}
	}
	...
	public <T> T getCapability(Capability<T> capability, @Nullable EnumFacing side){
		if(capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY && world.getBlockState(pos).getValue(EssentialsProperties.REDSTONE_BOOL) && connectMode != null && (side == null || connectMode[side.getIndex()] != 0)){
