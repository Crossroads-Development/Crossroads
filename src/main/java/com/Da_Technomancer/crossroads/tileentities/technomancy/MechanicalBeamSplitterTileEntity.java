...
public class MechanicalBeamSplitterTileEntity extends BeamRenderTE implements ITickable, IIntReceiver{
	...
	public void update(){
		...
		if(worldObj.getTotalWorldTime() % IMagicHandler.BEAM_TIME == 0){
			...
			double splitRatio = MiscOp.safeHasCap(worldObj, pos.offset(facing), Capabilities.AXLE_HANDLER_CAPABILITY, facing.getOpposite()) ? Math.min(Math.abs(worldObj.getTileEntity(pos.offset(facing)).getCapability(Capabilities.AXLE_HANDLER_CAPABILITY, facing.getOpposite()).getMotionData()[0]), 15D) / 15D : 0;
