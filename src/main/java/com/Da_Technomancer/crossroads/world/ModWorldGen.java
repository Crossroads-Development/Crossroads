	...
	public static void register(IForgeRegistry<Feature<?>> reg){
		...
		for(Biome biome : ForgeRegistries.BIOMES){
			...
			if(CRConfig.genRubyOre.get() && biome.getCategory() == Biome.Category.NETHER){
				biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Biome.createDecoratedFeature(GEM_FEATURE, new SingleGen.SingleOreConfig(SingleGen.SingleOreConfig.CRFillerType.QUARTZ, OreSetup.oreRuby.getDefaultState()), Placement.COUNT_RANGE, new CountRangeConfig(CRConfig.rubyRarity.get(), 8, 8, 118)));
			}
