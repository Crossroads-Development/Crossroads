	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
	    ...
	    case -1: //Nether
	    	...
	    	if(ModConfig.getConfigBool(genRubyOre))
	    		//The reason the spawn attempts is so high for rubies is that it can only generate in quartz ore. The average number of quartz ore per chunk divided by the number of blockspaces in the given height range (heights nether quartz spawns at) is about 1/350, so 1000 tries will give an average of about 1 ruby per chunk. Happy Mining!
