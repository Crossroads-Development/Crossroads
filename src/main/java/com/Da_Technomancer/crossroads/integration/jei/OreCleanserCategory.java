...
public class OreCleanserCategory implements IRecipeCategory<OreCleanserRec>{
	...
	protected OreCleanserCategory(IGuiHelper guiHelper){
		...
		icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(CRBlocks.oreCleanser, 1));
	}
	...
	public void setRecipe(IRecipeLayoutBuilder builder, OreCleanserRec recipe, IFocusGroup focuses){
		...
		builder.addSlot(RecipeIngredientRole.INPUT, 35, 31).addIngredient(ForgeTypes.FLUID_STACK, new FluidStack(CRFluids.steam.still, OreCleanserTileEntity.WATER_USE)).setFluidRenderer(1000, true, 16, 64).setOverlay(fluidOverlay, 0, 0);
		builder.addSlot(RecipeIngredientRole.OUTPUT, 111, 51).addItemStack(recipe.getResultItem());
		builder.addSlot(RecipeIngredientRole.OUTPUT, 131, 31).addIngredient(ForgeTypes.FLUID_STACK, new FluidStack(CRFluids.dirtyWater.still, OreCleanserTileEntity.WATER_USE)).setFluidRenderer(1000, true, 16, 64).setOverlay(fluidOverlay, 0, 0);
	}
}
