...
public class ModelGearOctagon{
	...
	/**
	 * Rotate angle first, then facing, then translate to position + .5 blocks in x,y,z.
	 */
	public void render(ResourceLocation res, Color color){
		
		float sHalf = 7F / (16F * (1F + (float) Math.sqrt(2F)));
		float top = -.375F;
		float bottom = -.5F;
		float lHalf = .4375F;
		
		float lHalfT = .5F;
		float sHalfT = .5F / (1F + (float) Math.sqrt(2F));
