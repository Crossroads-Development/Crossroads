...
public class Wheezewort extends DoublePlantBlock implements IGrowable{
	...
	public Wheezewort(){
		super(AbstractBlock.Properties.of(Material.PLANT).randomTicks().instabreak().sound(SoundType.GRASS));
...
//		CRBlocks.blockAddQue(this); No item form. Seeds are a separate item
	...
	@Override
	public boolean isBonemealSuccess(World world, Random p_180670_2_, BlockPos pos, BlockState state){
		return true;
	}
	...
	private void coldSpurt(ServerWorld world, BlockPos pos){
		CRSounds.playSoundServer(world, pos, CRSounds.STEAM_RELEASE, SoundCategory.BLOCKS, 0.75F, 0.5F + world.random.nextFloat());
	...
	@Override
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader world, List<ITextComponent> tooltips, ITooltipFlag context){
		tooltips.add(new TranslationTextComponent("tt.crossroads.wheezewort.purpose", COOLING));
		tooltips.add(new TranslationTextComponent("tt.crossroads.wheezewort.age"));
		tooltips.add(new TranslationTextComponent("tt.crossroads.wheezewort.dispense"));
		tooltips.add(new TranslationTextComponent("tt.crossroads.wheezewort.quip").setStyle(MiscUtil.TT_QUIP));
	}
}
