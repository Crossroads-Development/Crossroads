...
public class CRRenderTypes extends RenderType{
	...
	public static void stitchTextures(TextureStitchEvent.Pre event){
		...
		if(event.getAtlas().location().equals(TextureAtlas.LOCATION_BLOCKS)) {
			...
			event.addSprite(GEAR_8_RIM_TEXTURE);
			event.addSprite(GEAR_8_TOOTH_TEXTURE);
			event.addSprite(GEAR_24_TEXTURE);
			event.addSprite(GEAR_24_RIM_TEXTURE);
			event.addSprite(GEAR_24_TOOTH_TEXTURE);
			event.addSprite(AXLE_MOUNT_RIM_TEXTURE);
			event.addSprite(CLUTCH_END_TEXTURE);
