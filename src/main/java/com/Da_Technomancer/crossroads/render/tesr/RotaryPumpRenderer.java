...
public class RotaryPumpRenderer implements BlockEntityRenderer<RotaryPumpTileEntity>{
	...
	public void render(RotaryPumpTileEntity te, float partialTicks, PoseStack matrix, MultiBufferSource buffer, int combinedLight, int combinedOverlay){
		...
		for(int i = 0; i < 2; i++){
			...
			if(opt.isPresent()){
				matrix.mulPose(Vector3f.YP.rotationDegrees((1F - 2F * i) * opt.orElseThrow(NullPointerException::new).getAngle(partialTicks)));
			}
			//Draw central axle
			...
			Quaternion rotation = Vector3f.YP.rotationDegrees(90 * (2F*i - 1F));
			Quaternion rotationToReverseStupidHardCodedBladeAngle = null;
			if(i == 1){
				rotationToReverseStupidHardCodedBladeAngle = Vector3f.ZP.rotation((float) Math.atan(1F / 3F));
			}
