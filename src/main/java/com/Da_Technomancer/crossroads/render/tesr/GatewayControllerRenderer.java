...
public class GatewayControllerRenderer extends EntropyRenderer<GatewayControllerTileEntity>{
	...
	public void render(GatewayControllerTileEntity frame, float partialTicks, PoseStack matrix, MultiBufferSource buffer, int combinedLight, int combinedOverlay){
		...
		Quaternionf segmentRotation = Axis.ZP.rotationDegrees(-45);
	...
	@Override
	public AABB getRenderBoundingBox(GatewayControllerTileEntity te){
		//Increase render BB to include links and the entire formed frame
		return new AABB(te.getBlockPos()).inflate(Math.max(te.getRange(), te.isActive() ? te.getSize() : 0));
	}
}
