...
public class LightEffect implements IEffect{
	...
	public void doEffect(World worldIn, BlockPos pos, int mult, Direction dir){
		...
		if(state.getBlock() == CrossroadsBlocks.blockPureQuartz){
			worldIn.setBlockState(pos, CrossroadsBlocks.blockBrightQuartz.getDefaultState());
		}else if(state.getMaterial() == Material.ROCK && state.getBlock() != CrossroadsBlocks.blockBrightQuartz && !BeamManager.solidToBeams(state, worldIn, pos)){
		...
		}else if(dir != null && state.getBlock() != Blocks.GLOWSTONE && state.getBlock() != CrossroadsBlocks.blockBrightQuartz && state.getBlock() != Blocks.SEA_LANTERN && (state = worldIn.getBlockState(offsetPos = pos.offset(dir))).getBlock().isAir(state, worldIn, offsetPos)){
	...
	public static class VoidLightEffect implements IEffect{
		...
		public void doEffect(World worldIn, BlockPos pos, int mult, Direction dir){
			...
			}else if(state.getBlock() == CrossroadsBlocks.blockBrightQuartz){
