...
public final class AlchemyCore{
	...
	static{
		...
		// Reagents
		REAGENTS[0] = new BaseElementReagent("Phelostigen", -275D, -274D, 0, (EnumMatterPhase phase) -> PHELOSTIGEN_COLOR, null, 1, 2, new PhelostogenEffect((Double amount) -> (int) Math.min(8, Math.round(amount / 2D))), new BeamUnit(1, 0, 0, 0)){
			@Override
			public boolean isLockedFlame(){
				return true;
			}
		};
		REAGENTS[1] = new BaseElementReagent("Aether", -275D, -274D, 1, (EnumMatterPhase phase) -> CLEAR_COLOR, null, 1, 1, new AetherEffect(), new BeamUnit(0, 1, 0, 0));
		REAGENTS[2] = new BaseElementReagent("Adamant", Short.MAX_VALUE - 1, Short.MAX_VALUE, 2, (EnumMatterPhase phase) -> Color.GRAY, ModItems.adamant, 2, 0, null, new BeamUnit(0, 0, 1, 0));
		REAGENTS[3] = new StaticReagent("Sulfur", 115D, 445D, 3, (EnumMatterPhase phase) -> phase == EnumMatterPhase.GAS ? TRANSLUCENT_YELLOW_COLOR : phase == EnumMatterPhase.LIQUID ? Color.RED : Color.YELLOW, MiscUtil.oreDictPred("dustSulfur"), () -> MiscUtil.getOredictStack("dustSulfur", 1), 10, 0, null);
		...
		REAGENTS[6] = new StaticReagent("Mundane Salt", 800D, 1400D, 6, (EnumMatterPhase phase) -> phase == EnumMatterPhase.LIQUID ? Color.ORANGE : Color.WHITE, MiscUtil.oreDictPred("dustSalt"), () -> MiscUtil.getOredictStack("dustSalt", 1), 10, 0, SALT_EFFECT);// AKA table salt (sodium chloride).
		...
		REAGENTS[12] = new StaticReagent("Cinnabar", 580D, Short.MAX_VALUE, 12, (EnumMatterPhase phase) -> Color.RED, MiscUtil.oreDictPred("dustRedstone"), () -> MiscUtil.getOredictStack("dustRedstone", 1), 5, 0, null);// Mercury (II) sulfide.
		...
		REAGENTS[23] = new StaticReagent("Copper", 1000D, 2560D, 23, (EnumMatterPhase phase) -> Color.ORANGE, MiscUtil.oreDictPred("nuggetCopper"), () -> MiscUtil.getOredictStack("nuggetCopper", 1), 2, 0, null);
		REAGENTS[24] = new StaticReagent("Iron", 1500D, 2560D, 24, (EnumMatterPhase phase) -> phase == EnumMatterPhase.SOLID ? Color.GRAY : Color.RED, MiscUtil.oreDictPred("nuggetIron"), () -> MiscUtil.getOredictStack("nuggetIron", 1), 2, 0, null);
		REAGENTS[25] = new StaticReagent("Tin", 230D, 2560D, 25, (EnumMatterPhase phase) -> Color.LIGHT_GRAY, MiscUtil.oreDictPred("nuggetTin"), () -> MiscUtil.getOredictStack("nuggetTin", 1), 2, 0, null);
		REAGENTS[26] = new StaticReagent("Gunpowder", Short.MAX_VALUE - 1, Short.MAX_VALUE, 26, (EnumMatterPhase phase) -> Color.GRAY, MiscUtil.oreDictPred("gunpowder"), () -> MiscUtil.getOredictStack("gunpowder", 1), 10, 0, null);
		REAGENTS[27] = new StaticReagent("Ruby", 2000D, 3000D, 27, (EnumMatterPhase phase) -> Color.RED, MiscUtil.oreDictPred("gemRuby"), () -> MiscUtil.getOredictStack("gemRuby", 1), 10, 0, null);
		REAGENTS[28] = new StaticReagent("Emerald", 2000D, 3000D, 28, (EnumMatterPhase phase) -> Color.GREEN, MiscUtil.oreDictPred("gemEmerald"), () -> MiscUtil.getOredictStack("gemEmerald", 1), 10, 0, null);//Couldn't find actual figures on melting/boiling points of emerald/diamond, perhaps due to large variance.
		REAGENTS[29] = new StaticReagent("Diamond", 2000D, 3000D, 29, (EnumMatterPhase phase) -> Color.CYAN, MiscUtil.oreDictPred("gemDiamond"), () -> MiscUtil.getOredictStack("gemDiamond", 1), 10, 0, null);
		...
		//Dynamic reagents
		REAGENTS[32] = new ElementalReagent("Lumen", 32, (byte) 1, -275, -274, new LumenEffect(), false, new BeamUnit(32, 32, 32, 0), null);
		REAGENTS[33] = new ElementalReagent("Eldrine", 33, (byte) 1, -100, 350, new EldrineEffect(), true, new BeamUnit(32, 0, 32, 0), ModItems.solidEldrine);
		REAGENTS[34] = new ElementalReagent("Stasisol", 34, (byte) 1, 800, 1800, new StasisolEffect(), false, new BeamUnit(32, 16, 32, 0), ModItems.solidStasisol);
		REAGENTS[35] = new ElementalReagent("Fusas", 35, (byte) 2, Short.MAX_VALUE - 1, Short.MAX_VALUE, null, false, new BeamUnit(16, 32, 32, 0), ModItems.solidFusas, (IElementReagent) REAGENTS[34]);
		REAGENTS[36] = new ElementalReagent("Voltus", 36, (byte) 2, -275, -274, new VoltusEffect(), true, new BeamUnit(32, 32, 8, 0), null, (IElementReagent) REAGENTS[32]);
		...
		REAGENTS[38] = new StaticReagent("Densus", Short.MAX_VALUE - 1, Short.MAX_VALUE, 38, (EnumMatterPhase phase) -> Color.BLUE, MiscUtil.oreDictPred("gemDensus"), () -> MiscUtil.getOredictStack("gemDensus", 1), 20, 0, null);
		REAGENTS[39] = new StaticReagent("Anti-Densus", Short.MAX_VALUE - 1, Short.MAX_VALUE, 39, (EnumMatterPhase phase) -> Color.ORANGE, MiscUtil.oreDictPred("gemAntiDensus"), () -> MiscUtil.getOredictStack("gemAntiDensus", 1), 20, 0, null);
