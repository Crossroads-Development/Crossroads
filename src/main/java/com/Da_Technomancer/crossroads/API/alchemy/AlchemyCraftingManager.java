...
public final class AlchemyCraftingManager{
	...
	static{
		...
		REAGENTS[5] = new SimpleReagentType("Hydrogen Nitrate", -40D, 80D, 5, (MatterPhase phase) -> CLEAR_COLOR, null, 1, true, 0, null, SolventType.POLAR, 0, ACID_EFFECT);// Salt that forms nitric acid, AKA aqua fortis, in water.
		REAGENTS[6] = new SimpleReagentType("Sodium Chloride", 800D, 1400D, 6, (MatterPhase phase) -> phase == MatterPhase.LIQUID ? Color.ORANGE : Color.WHITE, ModItems.dustSalt, 100, true, 0, null, SolventType.POLAR, 0, SALT_EFFECT);// AKA table salt.
		REAGENTS[7] = new SimpleReagentType("Vanadium (V) Oxide", 690D, 1750D, 7, (MatterPhase phase) -> Color.YELLOW, ModItems.vanadiumVOxide, 100, true, 0, null, SolventType.POLAR, 0, null);// Vanadium (V) oxide. This should decompose at the specified boiling point, but there isn't any real point to adding that.
		REAGENTS[8] = new SimpleReagentType("Sulfur Sioxide", -72D, -10D, 8, (MatterPhase phase) -> CLEAR_COLOR, null, 1, true, 0, SolventType.POLAR, SolventType.POLAR, 0, null);
		REAGENTS[9] = new SimpleReagentType("Sulfur Trioxide", 20D, 40D, 9, (MatterPhase phase) -> phase == MatterPhase.SOLID ? TRANSLUCENT_WHITE_COLOR : CLEAR_COLOR, null, 1, true, 0, null, SolventType.POLAR, 0, null);
		REAGENTS[10] = new SimpleReagentType("Hydrogen Sulfate", 10D, 340D, 10, (MatterPhase phase) -> CLEAR_COLOR, null, 1, true, 0, null, SolventType.POLAR, 0, ACID_EFFECT);// Salt that forms sulfuric acid, AKA Oil of Vitriol, in water.
		REAGENTS[11] = new SimpleReagentType("Aqua Regia", -40D, 100D, 11, (MatterPhase phase) -> Color.ORANGE, null, 1, true, 0, SolventType.AQUA_REGIA, SolventType.POLAR, 0, new AquaRegiaAlchemyEffect());// Shouldn't really be its own substance (actually a mixture of nitric and hydrochloric acid), but the code is greatly simplified by making it a separate substance.
		REAGENTS[12] = new SimpleReagentType("Murcury (II) Sulfide", 580D, Short.MAX_VALUE, 12, (MatterPhase phase) -> Color.RED, Items.REDSTONE, 100, true, 0, null, null, 0, null);// Mercury (II) sulfide, AKA cinnabar.
		...
		REAGENTS[15] = new SimpleReagentType("Hydrogen Chloride", -110D, 90D, 15, (MatterPhase phase) -> CLEAR_COLOR, null, 1, true, 0, SolventType.POLAR, null, 0, ACID_EFFECT);// Salt that forms hydrochloric acid, AKA muriatic acid, in water. Boiling point should be -90, set to 90 due to the alchemy system not allowing gasses to dissolve. 
		REAGENTS[16] = new SimpleReagentType("Waste Salt", 900D, 1400D, 16, (MatterPhase phase) -> TRANSLUCENT_WHITE_COLOR, ModItems.wasteSalt, 1, true, 0, null, SolventType.POLAR, 0, new SaltAlchemyEffect());//Any salt byproduct that is too boring to bother adding. 
		...
		REAGENTS[18] = new SimpleReagentType("Philosopher Stone", Short.MAX_VALUE - 1, Short.MAX_VALUE, 18, (MatterPhase phase) -> Color.BLACK, ModItems.philosopherStone, 100, true, 1, null, null, 2, null);
		REAGENTS[19] = new SimpleReagentType("Practitioner Stone", Short.MAX_VALUE - 1, Short.MAX_VALUE, 19, (MatterPhase phase) -> Color.BLACK, ModItems.practitionerStone, 100, true, 2, null, null, 2, null);
		...
		REAGENTS[22] = new SimpleReagentType("Alchemical Crystal", Short.MAX_VALUE - 1, Short.MAX_VALUE, 22, (MatterPhase phase) -> CLEAR_COLOR, ModItems.alchCrystal, 10, true, 0, null, null, 0, null);
		...
		BASE_REACTIONS.add(new SimpleReaction(-35D, -50D, -20D, null, false, Triple.of(REAGENTS[2], 1D, MatterPhase.SOLID), Triple.of(REAGENTS[13], 4D, MatterPhase.LIQUID), null, Pair.of(REAGENTS[22], 5D), null, null));
