	...
	public static void releaseChemical(World world, BlockPos pos, ReagentMap reags){
		...
		if(hasFire){
			if(ModConfig.getConfigBool(ModConfig.phelEffect, false)){
				...
				if(prev.getBlock() == Blocks.FIRE || ModConfig.isProtected(world, pos, prev)){
					return;
				}
					...
					switch(phase){
						...
						case LIQUID:
							...
							for(int i = 0; i < 16; i++){
								...
								((WorldServer) world).spawnParticle(ModParticles.COLOR_SPLASH, false, pos.getX() + 0.5D, pos.getY() + 1.3F, pos.getZ() + 0.5D, 0, horizSpeed * Math.cos((2 * i + offset) * Math.PI / 8), (3 * offset + 1) * horizSpeed / 8D, horizSpeed * Math.sin((2 * i + offset) * Math.PI / 8), 1F, lcol.getRed(), lcol.getGreen(), lcol.getBlue(), lcol.getAlpha());
							}
							break;
						case SOLID:
							if(eff != null){
							...
							for(int i = 0; i < 5; i++){
								((WorldServer) world).spawnParticle(ModParticles.COLOR_SOLID, false, pos.getX() + 0.25D + world.rand.nextFloat() / 2F, pos.getY() + 1.3F, pos.getZ() + 0.25D + world.rand.nextFloat() / 2F, 0, 0, 0, 0, 1F, col.getRed(), col.getGreen(), col.getBlue(), col.getAlpha());
							}
							break;
						case FLAME:
						default:
							...
							Main.logger.error("Attempted to perform effect with invalid reagent phase %1$s and reagent %2$s.", phase, reag.getKey().getName());
							break;
					}
				}
			}
	...
	private static class QueuedEffect{
		...
		private void perform(World world, BlockPos pos, ReagentMap reags){
			...
			if(phase == EnumMatterPhase.GAS){
				((WorldServer) world).spawnParticle(ModParticles.COLOR_GAS, false, (float) pos.getX() + Math.random(), (float) pos.getY() + Math.random(), (float) pos.getZ() + Math.random(), 0, (Math.random() * 2D - 1D) * 0.015D, Math.random() * 0.015D, (Math.random() * 2D - 1D) * 0.015D, 1F, col.getRed(), col.getGreen(), col.getBlue(), col.getAlpha());
			}
		}
	}
}
