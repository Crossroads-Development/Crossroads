...
from PIL import Image  # You'll probably need to install 'pillow', as the original 'PIL' is discontinued
import colorsys
import math
...
def getOrdinal(xPos: int, yPos: int) -> int:
	...
	if distFromCentre > radius:
		return -1
	(r, g, b) = colorsys.hsv_to_rgb((math.atan2(yPos - y_center, xPos - x_center) / (2 * math.pi) + 1.0) % 1.0, distFromCentre / radius, 1)
...
for x in range(0, x_scale):
	for y in range(0, y_scale):
		...
		if ordinal >= 0:
			...
			endPos = ((ordinal % x_grid_size) * x_scale + x, (ordinal // x_grid_size) * y_scale + y)
