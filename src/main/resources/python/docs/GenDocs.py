    ...
    for inDir, subDirList, fileList in os.walk(srcPath):
        ...
        for file in fileList:
            ...
            if ".txt" in file:
                ...
                with open(
                    outDir + file.replace(".txt", ".json"), mode="w+", encoding="utf-8"
                ) as fOut:
                    ...
                    if len(advancement) == 0:
                        for line in tempLines:
                            fOut.write(
                                line.replace("CAT", category)
                                .replace("NAME", name)
                                .replace("ICON", icon)
                                .replace("PAGES", pages)
                                .replace("SORT", sort)
                                .replace("PRIO", priority)
...
def parseLine(line: str, size: int, prevFormat: str) -> tuple[str, str, str, int]:
    """
    ...
    boboMult = (
        1.2  # Bolded 'bobo' text is thicker; multiplier for charcount on bold text
    )
    boboTag = "<bobo>"
    space = " "
        ...
        while wordEndInd < len(line):  # Finds entire words and adds them
            if line[wordEndInd] == space:
                ...
                currChars += (wordEndInd + 1) * (
                    boboMult if boboTag in activeFormat else 1
                )
                if currChars > size:
                    # Filled page
                ...
                if code in ["<item>", "<thing>", "<em>", "<bobo>"]:
                    ...
                    line = line[
                        len(code) :
                    ]  # Remove the formatting code from the source line
                elif code == "/$":
                    activeFormat = ""
                    currPage += "/$"
                    line = line[
...
def writeTextPage(output: str, text: str, lineSt: str, data: list[str]) -> str:
    """
...
def writeSpotlightPage(output: str, text: str, lineSt: str, data: list[str]) -> str:
    """
    ...
    output += (
        ...
        + "\t"
        + '"item": "'
        + (data[1] if len(data) > 1 else "minecraft:stick")
        + '",\n'
    )
    output += lineSt + '\t"text": "' + text.replace('"', '\\"') + '"\n'
    output += lineSt + "}"
...
def writeImagePage(output: str, text: str, lineSt: str, data: list[str]) -> str:
    """
...
def writeEntityPage(output: str, text: str, lineSt: str, data: list[str]) -> str:
    """
...
def parseBody(
    text: list[str], icon: str, title: str, indents: int
) -> tuple[str, list[int]]:
    """
    ...
    jsonWriters = {
        "text": writeTextPage,
        "spotlight": writeSpotlightPage,
        "image": writeImagePage,
        "entity": writeEntityPage,
    }
    ...
    for line in text:
        ...
        if line.startswith("<page|"):
            ...
            parts = [
                x if x is not None else "" for x in line[6:-2].split("|")
            ]  # Sanitize the input, replace None entries with empty string
            pageType = parts[0]
            ...
            while len(lineText) > 0:
                (newPageText, lineText, pageFormat, pageSpaceRemain) = parseLine(
                    lineText, pageSpaceRemain, pageFormat
                )
...
def formatMappings(
    mappings: list[tuple[str, int]], lineStartPageNumbers: list[int], indent: int = 0
) -> str:
    """
    ...
    out = f",\n{leadingTabs}".join(
        f'"{mapping[0]}" : {lineStartPageNumbers[int(mapping[1])]}'
