...
with open("../assets/crossroads/lang/en_us.lang", "r+") as f:
    ...
    prefixed = [
        line.replace("tile.", "blocks.", 1) if line.startswith("tile.") else line
        for line in lines
    ]

    id = [
        line.replace(".", ".crossroads.", 1)
        if line.startswith("blocks.") or line.startswith("item.")
        else line
    ...
    cut = [
        line.replace(".name", "", 1)
        if (not line.startswith("info.") and not line.startswith("lore."))
        else line
        for line in id
    ]
