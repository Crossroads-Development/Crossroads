	...
	with open(templatePath + model + ".json", 'r') as template:
		for conductor in conductors:
			for insulator in insulators:
				with open(modelOutPath + model + insulator + '_' + conductor + '.json', 'w+') as fileOut:
					lines = template.readlines()
					template.seek(0)
					lines = [line.replace("<TEXT>", 'crossroads:block/heatcable/' + insulator + '-' + conductor) for line in lines]
	...
	with open(templatePath + state + ".json", 'r') as template:
		...
		for insulator in insulators:
			with open(stateOutPath + state + insulator + '.json', 'w+') as fileOut:
				lines = template.readlines()
				template.seek(0)
				lines = [line.replace("<CORE-copper>", 'crossroads:block/conduit/heat_cable_core_' + insulator + '_copper' + suffix).replace("<CORE-iron>", 'crossroads:block/conduit/heat_cable_core_' + insulator + '_iron' + suffix).replace("<CORE-quartz>", 'crossroads:block/conduit/heat_cable_core_' + insulator + '_quartz' + suffix).replace("<CORE-diamond>", 'crossroads:block/conduit/heat_cable_core_' + insulator + '_diamond' + suffix).replace("<END-copper>", 'crossroads:block/conduit/heat_cable_top_' + insulator + '_copper' + suffix).replace("<END-iron>", 'crossroads:block/conduit/heat_cable_top_' + insulator + '_iron' + suffix).replace("<END-quartz>", 'crossroads:block/conduit/heat_cable_top_' + insulator + '_quartz' + suffix).replace("<END-diamond>", 'crossroads:block/conduit/heat_cable_top_' + insulator + '_diamond' + suffix) for line in lines]
				fileOut.writelines(lines)
